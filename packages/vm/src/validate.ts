...
function getIntent(stmt: Assignment | Query | Increment | Action): ResolutionIntent | null {
...
function checkDeviceAndRoom(
  ...
  if (roomSelector?.kind === "room") {
    ...
    if (!inRoom) {
      return error(stmt, `No device '${deviceType}' found in room '${roomSelector.name}'`);
    }
  }
...
function updateItRef(
  ...
  if (candidates.length > 1 && roomSelector?.kind === "wildcard") return candidates[0]!;
...
function validateDevicePath(
  ...
  if (!firstSeg) {
    return { errors: [error(stmt, "Invalid statement: empty path")], newItRef: itRef };
  }
  ...
  if (firstSeg.isVariable && firstSeg.identifier !== "it") {
    ...
    if (!varDef) {
      return {
        errors: [error(stmt, `Variable '$${firstSeg.identifier}' is not defined`)],
        newItRef: itRef,
      };
    }
  }
  ...
  if (intent) {
    const capErr = checkCapability(stmt, stmt.path, devices, shadowVars, itRef, intent);
    if (capErr) {
...
function validateSimpleCondition(
  ...
  if (firstSeg.isVariable && firstSeg.identifier !== "it") {
    if (!shadowVars.has(firstSeg.identifier)) {
      return { errors: [error(parentStmt, `Variable '$${firstSeg.identifier}' is not defined`)], newItRef: itRef };
    }
  }
  ...
  const isOneof = firstSeg.isVariable
...
function validateConditionExpr(
  ...
  const left = validateConditionExpr(cond.left, devices, shadowVars, itRef, parentStmt);
  const right = validateConditionExpr(cond.right, devices, shadowVars, left.newItRef, parentStmt);
  return {
    ...
    switch (stmt.kind) {
      ...
      case "if": {
        ...
        if (condResult.errors.length === 0) {
          ...
          const bodyResult = validateStatements(stmt.body, devices, bodyVars, bodyItRef);
          errors.push(...bodyResult.errors);

          if (stmt.elseBody) {
            const elseVars = new Map(shadowVars);
            const elseResult = validateStatements(stmt.elseBody, devices, elseVars, bodyItRef);
...
export function validateProgram(
  ...
  const result = validateStatements(program.statements, devices, shadowVars, itRef);
