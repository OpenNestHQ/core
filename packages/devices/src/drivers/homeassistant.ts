...
export class HADriver implements DeviceDriver {
  ...
  async init(globalConfig: Record<string, unknown>): Promise<void> {
    ...
    if (typeof token !== "string" || !token) {
      throw new Error("homeassistant driver requires a `token` in global config");
    }
  ...
  async getProperty(
    ...
    const props = deviceConfig["properties"] as Record<string, HAPropertyConfig> | undefined;
  ...
  async setProperty(
    ...
    if (propConfig.set_service && boolValue !== null) {
      [domain, service] = this.resolveBoolService(propConfig.set_service, boolValue);
  ...
  async executeAction(
    ...
    const actions = deviceConfig["actions"] as Record<string, HAActionConfig> | undefined;
...
function splitService(service: string): [string, string] {
  ...
  if (dot === -1) throw new Error(`Invalid service format: "${service}". Expected "domain.service".`);
