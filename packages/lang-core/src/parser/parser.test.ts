...
import { parseHomeDSL } from './parser.js'
import type {
  Assignment,
  CompoundCondition,
  Segment,
...
function assignment(
  path: Segment[],
  value: Parameters<typeof makeAssignmentValue>[0],
): Assignment {
  return { kind: 'assignment', path, value: makeAssignmentValue(value) }
}

function makeAssignmentValue(v: number | 'on' | 'off' | string): Value {
...
describe('parseHomeDSL', () => {
  describe('state assignments', () => {
    it('should parse tv.power = on', () => {
      expect(parseHomeDSL('tv.power = on')).toEqual(
        result([assignment([seg('tv'), seg('power')], 'on')]),
      )
    })

    it('should parse tv.power = off', () => {
      expect(parseHomeDSL('tv.power = off')).toEqual(
        result([assignment([seg('tv'), seg('power')], 'off')]),
      )
    })

    it('should parse with room selector', () => {
      expect(parseHomeDSL('tv[living_room].power = on')).toEqual(
        result([assignment([seg('tv', 'living_room'), seg('power')], 'on')]),
      )
    })

    it('should parse light[bedroom].brightness = 50', () => {
      expect(parseHomeDSL('light[bedroom].brightness = 50')).toEqual(
        result([assignment([seg('light', 'bedroom'), seg('brightness')], 50)]),
      )
    })

    it('should parse wildcard room selector', () => {
      expect(parseHomeDSL('light[*].power = off')).toEqual(
        result([assignment([seg('light', '*'), seg('power')], 'off')]),
      )
    })

    it('should parse thermostat.temperature = 21', () => {
      expect(parseHomeDSL('thermostat.temperature = 21')).toEqual(
        result([assignment([seg('thermostat'), seg('temperature')], 21)]),
      )
    })

    it('should parse identifier value', () => {
      expect(parseHomeDSL('tv.source = hdmi1')).toEqual(
        result([assignment([seg('tv'), seg('source')], 'hdmi1')]),
      )
    })

    it('should parse enum value with underscores', () => {
      expect(parseHomeDSL('tv.source = salle_de_bain')).toEqual(
        result([assignment([seg('tv'), seg('source')], 'salle_de_bain')]),
      )
    })

    it('should parse fan.speed = 2', () => {
      expect(parseHomeDSL('fan.speed = 2')).toEqual(
        result([assignment([seg('fan'), seg('speed')], 2)]),
      )
    })

    it('should parse blind.position = 100', () => {
      expect(parseHomeDSL('blind.position = 100')).toEqual(
        result([assignment([seg('blind'), seg('position')], 100)]),
      )
    })

    it('should parse switch.power = on', () => {
      expect(parseHomeDSL('switch.power = on')).toEqual(
        result([assignment([seg('switch'), seg('power')], 'on')]),
      )
    })
  })
  ...
  describe('context reference ($it)', () => {
    it('should parse $it.power = off', () => {
      expect(parseHomeDSL('$it.power = off')).toEqual(
        result([assignment([vseg('it'), seg('power')], 'off')]),
      )
    })

    it('should parse $it.volume = 20', () => {
      expect(parseHomeDSL('$it.volume = 20')).toEqual(
        result([assignment([vseg('it'), seg('volume')], 20)]),
      )
    })
  ...
  describe('queries', () => {
    it('should parse tv.power?', () => {
      expect(parseHomeDSL('tv.power?')).toEqual(
        result([{ kind: 'query', path: [seg('tv'), seg('power')] }]),
      )
    })

    it('should parse thermostat.temperature?', () => {
      expect(parseHomeDSL('thermostat.temperature?')).toEqual(
        result([
          { kind: 'query', path: [seg('thermostat'), seg('temperature')] },
        ]),
      )
    })

    it('should parse query with room selector', () => {
      expect(parseHomeDSL('tv[living_room].power?')).toEqual(
        result([
          { kind: 'query', path: [seg('tv', 'living_room'), seg('power')] },
        ]),
      )
    })
  })
  ...
  describe('increments', () => {
    it('should parse speaker.volume += 10', () => {
      expect(parseHomeDSL('speaker.volume += 10')).toEqual(
        result([
          {
            kind: 'increment',
    ...
    it('should parse $it.volume += 5', () => {
      expect(parseHomeDSL('$it.volume += 5')).toEqual(
        result([
          {
            kind: 'increment',
    ...
    it('should parse thermostat.temperature += 2', () => {
      expect(parseHomeDSL('thermostat.temperature += 2')).toEqual(
        result([
          {
            kind: 'increment',
  ...
  describe('actions', () => {
    it('should parse vacuum.start()', () => {
      expect(parseHomeDSL('vacuum.start()')).toEqual(
        result([{ kind: 'action', path: [seg('vacuum'), seg('start')] }]),
      )
    })

    it('should parse camera.snapshot()', () => {
      expect(parseHomeDSL('camera.snapshot()')).toEqual(
        result([{ kind: 'action', path: [seg('camera'), seg('snapshot')] }]),
      )
    })

    it('should parse door.lock()', () => {
      expect(parseHomeDSL('door.lock()')).toEqual(
        result([{ kind: 'action', path: [seg('door'), seg('lock')] }]),
      )
    })

    it('should parse door.unlock()', () => {
      expect(parseHomeDSL('door.unlock()')).toEqual(
        result([{ kind: 'action', path: [seg('door'), seg('unlock')] }]),
      )
    })

    it('should parse speaker.play()', () => {
      expect(parseHomeDSL('speaker.play()')).toEqual(
        result([{ kind: 'action', path: [seg('speaker'), seg('play')] }]),
      )
    })

    it('should parse speaker.pause()', () => {
      expect(parseHomeDSL('speaker.pause()')).toEqual(
        result([{ kind: 'action', path: [seg('speaker'), seg('pause')] }]),
      )
    })

    it('should parse speaker.next()', () => {
      expect(parseHomeDSL('speaker.next()')).toEqual(
        result([{ kind: 'action', path: [seg('speaker'), seg('next')] }]),
      )
    })

    it('should parse action with room selector', () => {
      expect(parseHomeDSL('tv[living_room].play()')).toEqual(
        result([
          { kind: 'action', path: [seg('tv', 'living_room'), seg('play')] },
        ]),
      )
    })

    it('should parse $it.play()', () => {
      expect(parseHomeDSL('$it.play()')).toEqual(
  ...
  describe('variable assignments', () => {
    it('should parse $living_tv = tv[living_room]', () => {
      expect(parseHomeDSL('$living_tv = tv[living_room]')).toEqual(
        result([
          {
    ...
    it('should parse $lights = @all(light[living_room])', () => {
      expect(parseHomeDSL('$lights = @all(light[living_room])')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'lights',
    ...
    it('should parse $tvs = @all(tv[living_room])', () => {
      expect(parseHomeDSL('$tvs = @all(tv[living_room])')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'tvs',
    ...
    it('should parse @first collection', () => {
      expect(parseHomeDSL('$first_light = @first(light[bedroom])')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'first_light',
    ...
    it('should parse @oneof collection', () => {
      expect(parseHomeDSL('$my_tv = @oneof(tv[living_room])')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'my_tv',
    ...
    it('should parse @oneof without room selector', () => {
      expect(parseHomeDSL('$main_light = @oneof(light)')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'main_light',
    ...
    it('should parse @oneof with wildcard', () => {
      expect(parseHomeDSL('$any_tv = @oneof(tv[*])')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'any_tv',
    ...
    it('should parse collection with wildcard', () => {
      expect(parseHomeDSL('$all_lights = @all(light[*])')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'all_lights',
    ...
    it('should parse device ref without room', () => {
      expect(parseHomeDSL('$my_tv = tv')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'my_tv',
    ...
    it('should parse variable assigned to power value', () => {
      expect(parseHomeDSL('$state = on')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'state',
    ...
    it('should parse variable assigned to number', () => {
      expect(parseHomeDSL('$target_temp = 22')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'target_temp',
    ...
    it('should parse variable assigned to string', () => {
      expect(parseHomeDSL('$label = "living room"')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'label',
  ...
  describe('variable references in paths', () => {
    it('should parse $tv.power = on as assignment with isVariable', () => {
      expect(parseHomeDSL('$tv.power = on')).toEqual(
        result([assignment([vseg('tv'), seg('power')], 'on')]),
      )
    })

    it('should parse $tv.volume += 10 as increment with isVariable', () => {
      expect(parseHomeDSL('$tv.volume += 10')).toEqual(
        result([
          {
            kind: 'increment',
    ...
    it('should parse $tv.volume? as query with isVariable', () => {
      expect(parseHomeDSL('$tv.volume?')).toEqual(
        result([
          {
            kind: 'query',
            path: [vseg('tv'), seg('volume')],
    ...
    it('should parse $tv.play() as action with isVariable', () => {
      expect(parseHomeDSL('$tv.play()')).toEqual(
        result([
          {
            kind: 'action',
    ...
    it('should parse $salon_tv.power = off', () => {
      expect(parseHomeDSL('$salon_tv.power = off')).toEqual(
        result([assignment([vseg('salon_tv'), seg('power')], 'off')]),
      )
    })
  })
  ...
  describe('variable name validation', () => {
    it('should reject room selector on variable reference', () => {
      ...
      expect(r.errors[0]!.message).toMatch(
        /Variable references cannot have a room selector/,
      )
    })
    ...
    it('should parse variable with underscore and digits', () => {
      expect(parseHomeDSL('$tv_42 = tv[living_room]')).toEqual(
        result([
          {
            kind: 'variable_assignment',
            name: 'tv_42',
  ...
  describe('whitespace handling', () => {
    it('should trim leading whitespace', () => {
      expect(parseHomeDSL('   tv.power = on').program.statements).toHaveLength(
        1,
      )
    })
  ...
  describe('conditional blocks (@if)', () => {
    ...
    it('should record error for @if with invalid condition syntax', () => {
      const r = parseHomeDSL(
        '@if something without proper syntax\nlight.power = on\n@endif',
      )
      expect(r.errors.length).toBeGreaterThanOrEqual(1)
    })
    ...
    it('should handle @if condition with quoted string value', () => {
      ...
      if (r.program.statements[0]!.kind === 'if') {
        expect(
          (r.program.statements[0]!.condition as SimpleCondition).value.kind,
        ).toBe('string')
        if (
          (r.program.statements[0]!.condition as SimpleCondition).value.kind ===
          'string'
          ...
          expect(
            (r.program.statements[0]!.condition as SimpleCondition).value.value,
          ).toBe('hdmi1')
        }
      }
    })
    ...
    it('should parse @if condition with variable path segments', () => {
      ...
      const stmt = r.program.statements[0]!
      if (stmt.kind === 'if') {
        expect((stmt.condition as SimpleCondition).path[0]!.isVariable).toBe(
          true,
        )
        expect((stmt.condition as SimpleCondition).path[0]!.identifier).toBe(
          'light_salon',
        )
        expect((stmt.condition as SimpleCondition).path[1]!.identifier).toBe(
          'power',
        )
        expect((stmt.condition as SimpleCondition).op).toBe('==')
        expect((stmt.condition as SimpleCondition).value.kind).toBe('power')
      }
    })
    it('should parse @if with compound & condition', () => {
      const r = parseHomeDSL(
        `@if $a.power? == on & $b.power? == off\nspeaker.power = on\n@endif`,
      )
      expect(r.errors).toHaveLength(0)
      expect(r.program.statements).toHaveLength(1)
      const stmt = r.program.statements[0]!
      if (stmt.kind === 'if') {
        expect((stmt.condition as CompoundCondition).kind).toBe(
          'compound_condition',
        )
        if (
          (stmt.condition as CompoundCondition).kind === 'compound_condition'
        ) {
          expect((stmt.condition as CompoundCondition).operator).toBe('&')
          expect((stmt.condition as CompoundCondition).left.kind).toBe(
            'condition',
          )
          expect((stmt.condition as CompoundCondition).right.kind).toBe(
            'condition',
          )
        }
      }
    ...
    it('should parse @if with three conditions joined by &', () => {
      const r = parseHomeDSL(
        `@if $a.power? == on & $b.power? == off & $c.power? == on\nlight.power = on\n@endif`,
      )
      expect(r.errors).toHaveLength(0)
    })
    ...
    it('should parse @if with room selectors in compound condition', () => {
      const r = parseHomeDSL(
        `@if light[living_room].power? == on & tv[bedroom].power? == on\nspeaker.power = on\n@endif`,
      )
      expect(r.errors).toHaveLength(0)
    })
  })
  ...
  describe('owner selectors', () => {
    ...
    it('should parse collection with owner selector', () => {
      ...
      if (
        ...
        stmt.value.kind === 'collection'
      ) {
        expect(stmt.value.device.selectors).toEqual([
          { kind: 'owner', name: 'Alice' },
        ])
      }
    })
