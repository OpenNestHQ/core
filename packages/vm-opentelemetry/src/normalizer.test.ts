...
import { NodeCompletedEvent, NodeStartedEvent } from './events.js'
...
describe('ExecutionEventNormalizer', () => {
  it('emits node.started and node.completed for a program', () => {
    ...
    const started = begin[0]! as NodeStartedEvent

    expect(begin).toHaveLength(1)
    expect(started.type).toBe('node.started')
    expect(started.kind).toBe('Program')
    expect(started.name).toBe('program')
    expect(started.timestamp).toBe(1000)
    expect(started.parentNodeId).toBeUndefined()

    const ended = end[0]! as NodeCompletedEvent

    expect(end).toHaveLength(1)
    expect(ended.type).toBe('node.completed')
    expect(ended.nodeId).toBe(started.nodeId)
    expect(ended.status).toBe('success')
    expect(ended.timestamp).toBe(1100)
  })
  ...
  it('middlewareParentId is consumed by next action, not leaked to subsequent actions', () => {
    ...
    const action2Begin = norm.consume({
      kind: 'action:begin',
      timestamp: 1080,
      actionKind: 'read_property',
      deviceId: 'light.salon',
      deviceName: 'Salon Light',
    })
    norm.consume({
      kind: 'action:end',
      timestamp: 1090,
