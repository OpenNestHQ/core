...
export function VMProvider({ children }: { children: ReactNode }) {
  ...
  if (!adapterRef.current) {
    ...
    dispatch({ type: "SET_POLICIES", policies: adapterRef.current.getPolicies() });
    dispatch({
      type: "SET_DSL",
      source: '# Tapez du HomeDSL ci-dessous\n# Exemple: light[salon].power = on',
    });
    dispatch({
      type: "ADD_MESSAGE",
      message: buildChatMessage(
        ...
        "🏠 Bienvenue dans le Playground OpenNest !\n\n" +
          ...
          adapterRef.current.getDevices().map((d) =>
            `• ${d.name} (${d.type}, ${d.room})`
          ).join("\n"),
      ),
    });
    dispatch({
  ...
  const runDemo = useCallback(async () => {
    dispatch({
      type: "ADD_MESSAGE",
      message: buildChatMessage("system", "🚀 Lancement du scénario de démo..."),
    });
